...
class PlannerInputBuilder:
    def build(
        ...
        system_prompt = (
            ...
            "7. 任务控制类：如“重新处理刚刚那个试卷”“重跑上一个任务”“再处理一次上一份”\n"
            "8. 控制类消息：如确认开始、驳回、重新上传\n"
            "\n"
            "你必须遵守以下规则：\n"
            "- 只有在需要改变系统状态时，才调用工具\n"
            ...
            "- 如果用户说“开始处理”，但当前材料未齐全（缺 blank_pdf 或 solution_pdf），不要调用 process_paper，应提示用户先上传材料 \n"
            "\n"
            ...
            "\n"
            "关于重跑、重新处理类问题，必须额外遵守以下规则：\n"
            "- 如果用户说“重新处理刚刚那个试卷”“重跑上一个任务”“再处理一次上一份”，这表示用户想基于上一条历史任务的材料重新创建一个新任务\n"
            "- 遇到这类意图时，应调用工具 rerun_last_task\n"
            "- 调用 rerun_last_task 时，tool_args 必须包含 chat_id 和 current_task_id\n"
            "- 这类操作的目标不是覆盖旧任务，而是基于旧材料创建一个新的待处理任务\n"
            "- 创建完成后，应引导用户确认是否开始处理\n"
            "- 如果 rerun_last_task 返回失败，不要只重复“无法重跑”，应明确告诉用户原因，并引导用户重新上传试卷 PDF 和解析 PDF，以创建新的任务继续处理。\n"
            "- 当历史任务的本地材料文件不存在时，默认策略不是恢复旧材料，而是建议用户重新上传材料并创建新任务。 \n"
            "关于“直接开始处理”类问题，必须遵守以下规则：\n"
            "- 如果用户说“开始处理”“直接开始”“不用确认了”“直接跑”，表示用户希望跳过确认阶段直接执行处理流程 \n"
            "- 仅当当前任务处于“确认阶段（waiting_confirmation）”时，才允许直接进入处理流程 \n"
            "- 此时应调用工具 process_paper \n"
            "- tool_args 必须包含：\n"
            "1) task_id（当前任务ID）\n"
            "2) work_dir（当前任务工作目录，如 runtime_data/tasks/{task_id}）\n"
            "- 不需要再次调用 manage_task 进行阶段推进，由 process_paper 内部完成后续流程 \n"
            "- 如果当前任务不在确认阶段，则不要调用 process_paper，而是解释当前状态 \n"
            "\n"
            "当前允许使用的工具有：\n"
            "- manage_task\n"
            "- ingest_materials\n"
            ...
            "4. rerun_last_task 用于基于上一条历史任务的材料创建一个新的待处理任务\n"
            "\n"
